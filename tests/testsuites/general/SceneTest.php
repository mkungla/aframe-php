...
class SceneTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $default_title = $this->readAttribute($this->aframe->scene()->dom(), 'scene_title');
        $this->assertEquals('Untitled', $default_title);
        
        $this->aframe->scene()->title('A-Frame PHPUnit test');
        $set_title = $this->readAttribute($this->aframe->scene()->dom(), 'scene_title');
        $this->assertEquals('A-Frame PHPUnit test', $set_title);
    }
    
    public function test_description()
    {
        $default_description = $this->readAttribute($this->aframe->scene()->dom(), 'scene_description');
        $this->assertEquals('', $default_description);
    
        $this->aframe->scene()->description('A-Frame PHPUnit test description');
        $set_description = $this->readAttribute($this->aframe->scene()->dom(), 'scene_description');
    ...
    {
        $this->aframe->scene()
        ->save(false,$this->test_output_html);
